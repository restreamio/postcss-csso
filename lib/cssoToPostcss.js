...
function listToPostcss(list) {
    ...
    list.each(function(node) {
        if (node.type === 'Raw' || node.type === 'Space') {
            ...
            var postcssNode = cssoToPostcss(node);
...
function cssoToPostcss(node) {
    ...
    switch (node.type) {
        case 'StyleSheet':
            ...
            postcssNode.nodes = listToPostcss(node.rules);
        ...
        case 'Atrule':
            ...
            postcssNode.nodes = children && listToPostcss(children);
        ...
        case 'Ruleset':
            ...
            postcssNode.nodes = listToPostcss(node.block.declarations);
...
module.exports = function(node) {
    ...
    if (typeof WeakSet === 'function') {
        // use weak set to avoid using the same original postcss node
        // in resulting tree, since nodes are changing on tree building
        used = new WeakSet();
    }

    result = cssoToPostcss(node);
    used = null;
